...
@section('title', 'Dashboard Gudang - Bonn DIG')
        ...
        <main class="flex-1 overflow-y-auto">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
                            ...
                            <div class="flex items-center justify-between">
                                <div>
                                    ...
                                    <p class="mt-2 text-gray-600 group-hover:text-purple-600 transition-colors duration-300">
                                        Selamat datang, <span class="font-semibold text-purple-600 group-hover:text-purple-700 transition-colors duration-300">{{ auth()->user()->name ?? 'Gudang' }}</span>
                                    </p>
                                    <p class="text-sm text-gray-500 mt-1 group-hover:text-gray-600 transition-colors duration-300">
                                        NIP: {{ auth()->user()->nip ?? '-' }} | Divisi: {{ auth()->user()->divisi ?? '-' }}
                                    </p>
                                </div>
                                <div class="text-right">
                                    <p class="text-sm text-gray-500 group-hover:text-gray-600 transition-colors duration-300">{{ now()->timezone('Asia/Jakarta')->format('l, d F Y H:i') }} WIB</p>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                    <a href="{{ route('gudang.barang.index') }}" class="group">
                    ...
                    <a href="{{ route('gudang.barang.create') }}" class="group">
                    ...
                    <a href="{{ route('gudang.bon.index') }}" class="group">
