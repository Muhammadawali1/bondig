...
<head>
    ...
    <title>Bon Barang - {{ $bonBarang->kode_bon }}</title>
    <style>
        @media print {
            @page {
                margin: 10mm;
...
<body>
    ...
    <div class="container">
        ...
        <div class="header" style="display: flex; align-items: center; gap: 30px; margin-bottom: 20px;">
            <img src="{{ asset('logo/kemenprin.png') }}" alt="Logo KEMENPRIN" style="width: auto; height: 80px; object-contain; margin-right: 10px;">
        ...
        <div class="info-section">
            <div class="info-row">
                ...
                <div class="info-value">{{ $bonBarang->kode_bon }}</div>
            </div>
            <div class="info-row">
                <div class="info-label">Unit Kerja</div>
                <div class="info-value">{{ $bonBarang->divisi }}</div>
            </div>
            <div class="info-row">
                <div class="info-label">Tanggal</div>
                <div class="info-value">{{ isset($tanggalCetak) ? $tanggalCetak->format('d F Y') : '' }}</div>
            </div>
            <div class="info-row">
                <div class="info-label">Keperluan</div>
                <div class="info-value">{{ $bonBarang->keterangan ?? '-' }}</div>
            </div>
        </div>
            ...
            <tbody>
                @foreach($bonBarang->details as $index => $detail)
                    <tr>
                        ...
                        <td class="volume">{{ $detail->jumlah_diminta }}</td>
                        <td class="volume">{{ $detail->jumlah_disetujui ?? 0 }}</td>
                        <td class="volume">{{ $detail->barang->satuan }}</td>
