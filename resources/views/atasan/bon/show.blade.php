...
@section('title', 'Detail Approval Bon Barang')
        ...
        <main class="flex-1 p-6">
            <div class="max-w-5xl mx-auto">
                <div class="flex items-center mb-6">
                    <a href="{{ route('atasan.bon.index') }}" class="text-gray-600 hover:text-gray-800 mr-3">
                ...
                @if($bonBarang->status === 'menunggu_atasan')
                    <form action="{{ route('atasan.bon.add-item', $bonBarang->id) }}" method="POST" class="p-6 bg-white shadow rounded-lg overflow-hidden mb-6">
                        @csrf
                    ...
                    <form action="{{ route('atasan.bon.approve', $bonBarang->id) }}" method="POST" id="approvalForm">
                        @csrf
                                        ...
                                        @foreach($bonBarang->details as $detail)
                                            <tr class="border-t">
                                                ...
                                                <td class="p-3">
                                                    ...
                                                    <input type="number" name="jumlah_disetujui[]" value="{{ $detail->jumlah_diminta }}" min="1" class="w-20 px-2 py-1 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" onchange="updateStatus(this)">
                                ...
                                <tbody>
                                    @foreach($bonBarang->details as $detail)
                                        <tr class="border-t">
                                            ...
                                            <td class="p-3">{{ $detail->jumlah_diminta }} {{ $detail->barang->satuan }}</td>
                                            <td class="p-3">
                                                {!! $detail->jumlah_disetujui ? $detail->jumlah_disetujui . ' ' . $detail->barang->satuan : '<span class="text-gray-400">-</span>' !!}
                                            </td>
                                            <td class="p-3">
                                            ...
                                            <td class="p-3">
                                                {!! $detail->catatan ? '<span class="text-gray-600">' . $detail->catatan . '</span>' : '<span class="text-gray-400">-</span>' !!}
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
...
<div id="rejectModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden overflow-y-auto h-full w-full z-50">
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
        <div class="mt-3">
            ...
            <form id="rejectForm" method="POST" action="{{ route('atasan.bon.reject', $bonBarang->id) }}">
                @csrf
                <div class="mt-4">
...
function confirmDelete(detailId) {
    if (confirm('Hapus barang ini dari bon?')) {
        ...
        form.method = 'POST';
        form.action = '{{ route("atasan.bon.remove-item", ":id") }}'.replace(':id', detailId);
